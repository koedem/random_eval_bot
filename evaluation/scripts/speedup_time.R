...
pdf(paste0(file_name, "_time.pdf"))
ggplot(data_trimmed, aes(x=num_thr, y=time_norm, group=ply)) +
  geom_line(aes(color=ply)) +
  scale_color_viridis_c() +
  scale_x_continuous(breaks = c(1, max_threads / 4, max_threads / 2, max_threads * 3 / 4, max_threads)) +
  labs(title = "Speedup (Time) vs Number of Threads",
       x = "Number of Threads",
       y = "Speedup (Normalized time)",
       color = "Ply")
